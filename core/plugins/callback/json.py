...
DOCUMENTATION =  """
  name: pureport.core.json
  type: stdout
  author: Pureport
  short_description: Displays output to stdout as JSON
  description:
    - Displays the output of plays and tasks as JSON to stdout
  options:
    working_dir:
      description: Working directory
      env:
        - name: PUREPORT_CLI_WORKING_DIR
      type: path
"""
...
class CallbackModule(CallbackBase):
    ...
    CALLBACK_NAME = "pureport.core.json"
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        working_dir = self.get_option('working_dir')
        if working_dir:
            with open(os.path.join(working_dir, 'output'), 'w') as f:
    ...
    def _record_task_result(self, on_info, result, **kwargs):
        ...
        self.results[-1]['tasks'][-1]['result'] = task_result
